    ...
    def probe_until_contact(
        ...
        while not rospy.is_shutdown():
            if rospy.Time.now() > deadline:
                self._robot_interface.stop_motion()
                return ProbeResult(False, None, self._get_wrench_snapshot(), "timeout")

            if self._is_wrench_stale():
                self._robot_interface.stop_motion()
                return ProbeResult(False, None, self._get_wrench_snapshot(), "stale_wrench")

            current_pose = self._tf_interface.get_tool_pose_in_base()
            if current_pose is None:
                self._robot_interface.stop_motion()
                return ProbeResult(False, None, self._get_wrench_snapshot(), "missing_tf")
            ...
            if travel_distance >= max(0.0, max_travel):
                self._robot_interface.stop_motion()
                return ProbeResult(False, None, self._get_wrench_snapshot(), "max_travel_reached")
            ...
            if self._contact_detector.detect_contact_along_axis(axis_name, threshold):
                ...
                return ProbeResult(True, contact_point, wrench_snapshot, "contact_detected")
        ...
        self._robot_interface.stop_motion()
        return ProbeResult(False, None, self._get_wrench_snapshot(), "shutdown")

...
def main() -> None:
    ...
    camera_frame = str(
        rospy.get_param("~camera_frame", "zedm_left_camera_optical_frame")
    ).strip()
    topic = str(
        rospy.get_param("~topics/camera_pose_debug", "/usb_c_insertion/camera_pose")
    ).strip()
    publish_rate = float(rospy.get_param("~camera_pose_publish_rate", 10.0))

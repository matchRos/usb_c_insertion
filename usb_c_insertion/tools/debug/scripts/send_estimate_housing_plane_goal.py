...
class EstimateHousingPlaneGoalClient:
    def __init__(self):
        self._action_name = str(get_param("~housing_plane/action_name", "estimate_housing_plane")).strip()
        self._wait_timeout = float(get_param("~housing_plane/client_wait_timeout", 5.0))
        self._result_timeout = float(
            get_param(
                "~housing_plane/client_result_timeout",
                get_param("~housing_plane/timeout", 3.0) + 2.0,
            )
        )
    ...
    def run(self) -> bool:
        ...
        rospy.loginfo(
            ...
            goal.image_topic,
            goal.cloud_topic,
            str(get_param("~housing_plane/base_transform_frame", "")).strip() or "<cloud_frame>",
            str(get_param("~housing_plane/cloud_to_base_transform_rotation", "identity")).strip(),
    ...
    def _build_goal(self) -> EstimateHousingPlaneGoal:
        ...
        goal.image_topic = str(
            get_param("~image_topic", get_param("~housing_plane/image_topic", "/zedm/zed_node/left/image_rect_color"))
        ).strip()
        goal.cloud_topic = str(
            get_param("~cloud_topic", get_param("~housing_plane/cloud_topic", "/zedm/zed_node/point_cloud/cloud_registered"))
        ).strip()
        goal.timeout = float(get_param("~housing_plane/timeout", 3.0))
        goal.min_blob_area = float(get_param("~housing_plane/min_blob_area", 120.0))
        goal.roi_radius_px = int(get_param("~housing_plane/roi_radius_px", 70))
        goal.roi_stride_px = int(get_param("~housing_plane/roi_stride_px", 2))
        goal.depth_window_m = float(get_param("~housing_plane/depth_window_m", 0.06))
        goal.ransac_iterations = int(get_param("~housing_plane/ransac_iterations", 120))
        goal.ransac_distance_threshold = float(get_param("~housing_plane/ransac_distance_threshold", 0.004))
        goal.min_inliers = int(get_param("~housing_plane/min_inliers", 120))
        goal.use_svd_refit = bool(get_param("~housing_plane/use_svd_refit", True))
        goal.use_largest_component = bool(get_param("~housing_plane/use_largest_component", True))
        return goal
